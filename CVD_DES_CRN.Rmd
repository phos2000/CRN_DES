...
patientWcea %>%
  kable(caption = "traces of simulated individuals with undiscounted cost and utilities") %>%
  kable_styling() %>%
  scroll_box(height = "400px")
...
#   
#   traces1 = o1$results %>% select(-start_time, -activity_time, -replication) %>% spread(resource, end_time) %>% 
#   left_join(casted1 %>% select(name, strategy, statins_use = aUseStatins, initial_age = AgeInitial), by = c("name", "strategy"))
# 
#   if (is.null(traces1$statins_major_adverse_event)) {traces1$statins_major_adverse_event = NA}
# 
...
#   
#   traces2 = o2$results %>% select(-start_time, -activity_time, -replication) %>% spread(resource, end_time) %>% 
#   left_join(casted2 %>% select(name, strategy, statins_use = aUseStatins, initial_age = AgeInitial), by = c("name", "strategy"))
# 
#   if (is.null(traces2$statins_major_adverse_event)) {traces2$statins_major_adverse_event = NA}
#   
# which(traces1 != traces2, arr.ind = TRUE)
#   
#   patientWcea = traces %>% 
#     rowwise() %>%
#     mutate(
...
for (vN in 10^(2:5)){
  ...
  converge = mclapply(1:10, mc.cores = n_cores, function(i){
...
save(params_CRN, file = "params_CRN.RData")
```